...
def build_input_fn(builder, global_batch_size, topology, is_training):
  ...
  def _input_fn(input_context):
    ...
    # update num_classes if custom dataset    
    if FLAGS.num_classes > 0:
        num_classes = FLAGS.num_classes

    def map_fn_train(example):
        """Produces multiple transformations of the same batch."""
        try:
            image, label, index = example['image'], example['label'], example['id']
            index = tf.strings.to_number(tf.strings.regex_replace(index, '[^0-9]', '', replace_global=True), tf.int32)
        except:
            image, label, index = example['image/encoded'], example['image/class/label'], example['image/ID']
    ...
    dataset = builder.as_dataset(
        ...
        as_supervised=False if is_training else True, # return id, image, label
    ...
    if is_training:
        dataset = dataset.map(map_fn_train, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    else:
        dataset = dataset.map(map_fn_test, num_parallel_calls=tf.data.experimental.AUTOTUNE)
