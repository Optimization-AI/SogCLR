...
class BatchIndependentLoss(tf.losses.Loss):
    ...
    def __call__(self,
      ...
      logits_aa = logits_aa - masks * LARGE_NUM
      logits_bb = tf.matmul(hidden2, hidden2_large, transpose_b=True) 
      logits_bb = logits_bb - masks * LARGE_NUM
      logits_ab = tf.matmul(hidden1, hidden2_large, transpose_b=True)
      logits_ba = tf.matmul(hidden2, hidden1_large, transpose_b=True)  
          
      #SogCLR
      neg_mask = 1-labels
      logits_ab_aa = tf.concat([logits_ab, logits_aa ], 1) 
      logits_ba_bb = tf.concat([logits_ba, logits_bb ], 1) 
      
      neg_logits1 = tf.math.exp(logits_ab_aa/temperature)*neg_mask   #(B, 2B)
      neg_logits2 = tf.math.exp(logits_ba_bb/temperature)*neg_mask
      data_index = tf.cast(data_index, tf.int32)

      u_i1 = (1 - gamma) * tf.gather(self.u, data_index) + gamma *  tf.math.divide(tf.math.reduce_sum(neg_logits1, axis=1, keepdims=True), tf.cast(2*(batch_size-1), dtype=(tf.float32)))
      u_i2 = (1 - gamma) * tf.gather(self.u, data_index) + gamma *  tf.math.divide(tf.math.reduce_sum(neg_logits2, axis=1, keepdims=True), tf.cast(2*(batch_size-1), dtype=(tf.float32)))

      # this sync on all devices (since "hidden" are gathering from all devices)
      if strategy is not None:
         u_i1_large = tpu_cross_replica_concat(u_i1, strategy)
         u_i2_large = tpu_cross_replica_concat(u_i2, strategy)
