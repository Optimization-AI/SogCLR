...
flags.DEFINE_integer(
    'num_classes', 0,
    'Force to assign number of classes if custom dataset'
    '0: do nothing')
...
flags.DEFINE_float(
    'crop_area_min', 0.08,
    'The min value of random cropped region.')
...
def perform_evaluation(model, cur_step, builder, eval_steps, ckpt, strategy, topology, summary_writer=None, summary_off=False):
  """Perform evaluation."""
  ...
  if not summary_writer and not summary_off:
  ...
  with strategy.scope():
    ...
    logging.info('Performing eval at step %d', cur_step)
...
def main(argv):
  ...
  try:
      num_classes = builder.info.features['image/class/label'].num_classes
  except:
  ...
  if FLAGS.use_tpu:
    if FLAGS.tpu_name:
      cluster = tf.distribute.cluster_resolver.TPUClusterResolver(FLAGS.tpu_name, zone=FLAGS.tpu_zone, project=FLAGS.gcp_project)
    else:
  ...
  with strategy.scope():
    ...
    if FLAGS.DCL_mode:
        DCL_Loss = obj_lib.DynamicContrastiveLoss(N=num_train_examples)

  if FLAGS.mode == 'eval':
      # check latest checkpoint 
    for ckpt in tf.train.checkpoints_iterator(FLAGS.model_dir, min_interval_secs=15):
      cur_step = int(ckpt.split('-')[-1])
      perform_evaluation(model, cur_step, builder, eval_steps, ckpt, strategy, topology, summary_writer=None, summary_off=True)
    ...
    def single_step(features, labels, index=None, u=None):
    ...
    def train_multiple_steps(iterator):
        for _ in tf.range(steps_per_loop):
          with tf.name_scope(''):
                images, labels, index = next(iterator)
                features, labels = images, {'labels': labels}
                strategy.run(single_step, (features, labels, index))
    ...
    with strategy.scope():
      ...
      while cur_step < train_steps:
        with summary_writer.as_default():
          ...
          logging.info('Step: [%d] %s = %f', cur_step, 'train/learning_rate',  learning_rate(tf.cast(global_step, dtype=tf.float32)).numpy())
          logging.info('Step: [%d] %s = %f', cur_step, 'train/gamma',  FLAGS.gamma)
          metrics.log_and_write_metrics_to_summary(all_metrics, cur_step)
