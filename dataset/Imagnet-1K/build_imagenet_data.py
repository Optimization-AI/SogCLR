...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames, file_id_dict,
  ...
  for s in xrange(num_shards_per_batch):
    ...
    # re-do 400-499; 500-599
    #if shard < 400 and shard> 600:
    #   print ('skip\n\n\n\n\n')
    #   continue      
    ...
    for i in files_in_shard:
      ...
      UID = file_id_dict[filename]
      example = _convert_to_example(filename, UID, image_buffer, label,
                                    synset, human, bbox,
                                    height, width)
...
def _process_image_files(name, filenames, file_id_dict, synsets, labels, humans,
                         bboxes, num_shards):
  ...
  for thread_index in xrange(len(ranges)):
    #if thread_index not in [3,4]:
    #   continue
    args = (coder, thread_index, ranges, name, filenames, file_id_dict, 
            synsets, labels, humans, bboxes, num_shards)
